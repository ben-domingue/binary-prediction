...
simfun<-function(df.tr,df.test,fm1,fm2) {
    ...
    ##
    tr<-list(prev=prev,ew=ew,aic1=m1$aic,aic0=m0$aic)
                                        #ew2=ew2,
             #r2=r2,
             #r2.del=r2.del,
             #auc=au,
...
pdf("/home/bd/Dropbox/Apps/Overleaf/BinaryPrediction/sim_aic.pdf",width=6,height=5)
par(mfrow=c(2,2),mgp=c(2,1,0),mar=c(3,3,1,1),oma=rep(.5,4),pch=19)
plot(mat.noz$b0,mat.noz$ew,xlab=expression(beta[0]),ylab="IMV")
plot(mat.noz$b0,mat.noz$aic1-mat.noz$aic0,xlab=expression(beta[0]),ylab="Change in AIC")
plot(mat.noz$b1,mat.noz$ew,ylab="IMV",xlab=expression(beta[1]))
plot(mat.noz$b1,mat.noz$aic1-mat.noz$aic0,ylab="Change in AIC",xlab=expression(beta[1]))
dev.off()


##                                         #pf(mat.noz$b0,mat.noz[,c("ew","r2","auc","f1")],xlab=expression(beta[0]),ylab="Metric")
## #pf(mat.noz$b1,mat.noz[,c("ew","r2","auc","f1")],xlab=expression(beta[1]),ylab="Metric")
## #dev.off()
...
## #pdf("/home/bd/Dropbox/Apps/Overleaf/BinaryPrediction/sim2B.pdf",width=6,height=4.5)
## #layout(matrix(c(1,1,2,3),2,2,byrow=FALSE))
## par(mfcol=c(2,3),mgp=c(2,1,0),mar=c(3,3,1,1),oma=rep(.5,4))
## pf(mat.wz1$b2,mat.wz1[,c("ew0","r2","auc","f1")],xlab=expression(beta[2]),ylab="Metric")
## legend("topleft",bty='n',legend=c(expression(paste(beta[0],"=0")),expression(paste(beta[1],"=0.5"))))
## pf(mat.wz1$b2,mat.wz1[,c("ew","r2.diff","auc.diff","f1.diff")],xlab=expression(beta[2]),ylab="Metric (difference)",ylim=c(0,.6))
## pf(mat.wz2$b2,mat.wz2[,c("ew0","r2","auc","f1")],xlab=expression(beta[2]),ylab="Metric")
## legend("topleft",bty='n',legend=c(expression(paste(beta[0],"=0.5")),expression(paste(beta[1],"=0.5"))))
## pf(mat.wz2$b2,mat.wz2[,c("ew","r2.diff","auc.diff","f1.diff")],xlab=expression(beta[2]),ylab="Metric (difference)",ylim=c(0,.6))
## pf(mat.wz3$b2,mat.wz3[,c("ew0","r2","auc","f1")],xlab=expression(beta[2]),ylab="Metric")
## legend("topleft",bty='n',legend=c(expression(paste(beta[0],"=0.5")),expression(paste(beta[1],"=0.1"))))
## pf(mat.wz3$b2,mat.wz3[,c("ew","r2.diff","auc.diff","f1.diff")],xlab=expression(beta[2]),ylab="Metric (difference)",ylim=c(0,.6))
## #dev.off()
