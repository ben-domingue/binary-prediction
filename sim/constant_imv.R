...
L<-list()
for (b0 in seq(0,1,length.out=10)) L[[as.character(b0)]]<-f(b0,omega=0.005)
df1<-data.frame(do.call("rbind",L))
L<-list()
for (b0 in seq(0,1,length.out=10)) L[[as.character(b0)]]<-f(b0,omega=.05)
...
plot(df2$b1,df2$r2,type='l',col='red')
lines(df1$b1,df1$r2,col='black')
legend("topleft",title="omega",legend=c(0.01,0.03),fill=c("black","red"))
