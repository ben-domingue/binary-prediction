...
library(parallel)
L<-mclapply(L,ff,mc.cores=28)
x<-data.frame(do.call("rbind",L))
save(x,file="/tmp/logreg.Rdata")

load("logreg.Rdata")
x$ln<-log10(x$N)
xx<-split(x,x$b)
pdf("/home/bd/Dropbox/Apps/Overleaf/BinaryPrediction/ben_figures/logreg.pdf",width=7,height=3)
...
for (i in 1:length(xx)) {
    ...
    y<-y[order(y$ln),]
    ##
    pf(y$ln,y$om,col=cols[1],expression(omega[0]))
    ##
    pf(y$ln,y$om.overfit,col=cols[2],"overfit")
    pf(y$ln,y$om.oracle,col=cols[3],"oracle")
}
dev.off()
