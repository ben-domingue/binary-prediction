...
pdf("/home/bd/Dropbox/Apps/Overleaf/BinaryPrediction/ben_figures/compare.pdf",width=5,height=5)
par(mgp=c(2,1,0),mar=c(3,3,1,1),oma=rep(.1,4))
plot(x,col='blue',pch=19,cex=.5)
dev.off()

plot(x$IMV,x$F1)
par(mfrow=c(1,2))
plot(x$Difference,x$F1)
points(x$Difference,x$IMV,col='red')
plot(x$Prevalence,x$F1)
points(x$Prevalence,x$IMV,col='red')
