##something awry here. need better way of handling all the variable types



df<-read.csv("agaricus-lepiota.data",header=FALSE)

x<-df[,-1]
names(x)<-paste("x",1:ncol(x),sep="")
y<-ifelse(df[,1]=="e",1,0)
df<-df<-data.frame(y,x)
df$x16<-NULL

nms<-paste(names(df)[-1])
